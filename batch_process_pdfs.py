...
def extract_images_from_pdf(pdf_path):
    ...
    for page_index in range(len(doc)):
        ...
        # Fallback to page text for context
        ...
        for img_index, img in enumerate(image_list):
            ...
            # FILTERS: Skip tiny icons (<1KB) or huge backgrounds (>3MB)
            ...
            image_filename = (
                f"{pdf_prefix}_p{page_index + 1}_img{img_index + 1}.{image_ext}"
            )
...
def analyze_images_with_groq(extracted_items):
    ...
    for item in extracted_items:
        try:
            ...
            3. If valid, write a 'dense_caption' describing the action being performed based on the image and the surrounding context text.
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        print(
                            f"      ⚠️  Error on {item['id']}, retrying in 5 seconds... ({e})"
                        )
                        time.sleep(5)
                    else:
                        raise e
...
def main():
    ...
    for i, pdf_file in enumerate(pdf_files):
        ...
        print(
            f"   💾 Saved {len(new_entries)} new entries from {pdf_file}. Total DB: {len(combined_data)}"
        )
