...
def generate_guide_from_rag(query):
    ...
    # --- DEBUGGING: PRINT EXACTLY WHAT WAS FOUND ---
    print(f"--- 🧐 DEBUG: Retrieved {len(relevant_docs)} chunks from DB ---")
    for i, doc in enumerate(relevant_docs):
        # We print the first 200 chars of each chunk to see what the DB thinks is relevant
        clean_content = doc.page_content.replace("\n", " ")[:200]
        filename = doc.metadata.get('filename', 'Unknown')
        print(f"   [Chunk {i+1} | Source: {filename}] -> {clean_content}...")
    # -----------------------------------------------
...
if __name__ == "__main__":
    # Test 1: Valid Query
    q1 = "Fix the leakage of the washing Machine"
